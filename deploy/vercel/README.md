...
A one-way Service Binding injects the private web URL as `WORKFLOW_SERVICE_URL`
into the API runtime. The web runtime uses Vercel's automatic `VERCEL_URL` to
reach the deployment's public `/api/v1/*` route. Each Preview deployment
therefore calls its matching Preview services without a circular binding.
...
`WORKFLOW_SERVICE_URL` is generated by the API-to-web Service Binding.
`VERCEL_URL` is a Vercel system variable used by the web BFF and Workflow steps
to reach the matching deployment's `/api/v1/*` route. `BACKEND_URL` remains a
local and standalone-deployment override. Explicit Workflow trigger variables
remain supported as legacy overrides.
...
|---|---|---|
| `web` | System `VERCEL_URL` | `NEXT_PUBLIC_GOOGLE_CLIENT_ID`, `GUEST_SIGNING_SECRET`, `INTERNAL_JOB_SECRET` |
| `api` | `WORKFLOW_SERVICE_URL` | `GOOGLE_CLIENT_ID`, `SECRET_KEY_ACCESS_API`, `GUEST_SIGNING_SECRET`, `INTERNAL_JOB_SECRET` |
