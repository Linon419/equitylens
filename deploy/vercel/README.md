...
runtime. A reverse binding injects the private web URL as
`WORKFLOW_SERVICE_URL` into the API runtime. Each Preview deployment therefore
calls its matching Preview services automatically.
...
`BACKEND_URL` and `WORKFLOW_SERVICE_URL` are generated by Service Bindings.
The explicit Workflow trigger variables remain supported as legacy overrides.
